...
uint16_t tpalet_original_codes[ 4 ];

// show help message
static void show_help_message() {
  printf("usage: rssn [options] <board-file.brd>\n");
  printf("options:\n");
  printf("     -s <speed> ... baud rate (9600/19200(default)/38400)\n");
...
int32_t main(int32_t argc, char* argv[]) {
  ...
  // board file name
  char* board_file_name = NULL;

  // baud rate
  int32_t baud_rate = 19200;

  // bar color
  uint16_t bar_color = (0b01111 << 11) | (0b01001 << 6) | (0b11111) << 1 | 1;
  ...
  for (int16_t i = 1; i < argc; i++) {
    if (argv[i][0] == '-' && strlen(argv[i]) >= 2) {
      if (argv[i][1] == 's' && i+1 < argc) {
        ...
        memcpy(cut_file_color, argv[i+1], 4);
        i++;
  ...
  // open rss board
  if (rss_board_open(&rss_board, board_file_name) != 0) {
    goto exit;
  }
  ...
  // open panel
  panel_open(&panel, &rss_board, cut_file_name, cut_file_color);

  // select channel
  RSS_CHANNEL* rss_channel = panel_select_channel(&panel);

  // download channel items
  if (rss_channel_download_items(rss_channel, &uart) != 0) {
    goto exit;
  }

  // select channel item
  panel_select_channel_item(&panel, rss_channel);
